...
namespace StellaFiesta.Client.CoreStandard
{
    public interface IHttpRequestExecutor
    {
        ...
        Task<TResponse> Post<TResponse>(string url, IEnumerable<KeyValuePair<string, string>> bodyParameter)
            where TResponse : class;

        Task<TResponse> Post<TResponse>(string url, byte[] bodyParameter)
            where TResponse : class;
